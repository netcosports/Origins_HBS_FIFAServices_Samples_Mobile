 ...
 */
extern BOOL RXTriviaAbortOnThreadingHazard;

/// Error domain for RXObjCRuntime.
extern NSString * __nonnull const RXObjCTriviaRuntimeErrorDomain;

/// `userInfo` key with additional information is interceptor probably KVO.
extern NSString * __nonnull const RXObjCTriviaRuntimeErrorIsKVOKey;

typedef NS_ENUM(NSInteger, RXObjCTriviaRuntimeError) {
    RXObjCTriviaRuntimeErrorUnknown                                           = 1,
    RXObjCTriviaRuntimeErrorObjectMessagesAlreadyBeingIntercepted             = 2,
    RXObjCTriviaRuntimeErrorSelectorNotImplemented                            = 3,
    RXObjCTriviaRuntimeErrorCantInterceptCoreFoundationTollFreeBridgedObjects = 4,
    RXObjCTriviaRuntimeErrorThreadingCollisionWithOtherInterceptionMechanism  = 5,
    RXObjCTriviaRuntimeErrorSavingOriginalForwardingMethodFailed              = 6,
    RXObjCTriviaRuntimeErrorReplacingMethodWithForwardingImplementation       = 7,
    RXObjCTriviaRuntimeErrorObservingPerformanceSensitiveMessages             = 8,
    RXObjCTriviaRuntimeErrorObservingMessagesWithUnsupportedReturnType        = 9,
};

/// Transforms normal selector into a selector with RX prefix.
SEL _Nonnull RX_TRIVIAselector(SEL _Nonnull selector);

/// Transforms selector into a unique pointer (because of Swift conversion rules)
void * __nonnull RX_TRIVIAreference_from_selector(SEL __nonnull selector);
...
IMP __nullable RX_TRIVIAensure_observing(id __nonnull target, SEL __nonnull selector, NSError *__autoreleasing __nullable * __nullable error);

#endif

/// Extracts arguments for `invocation`.
NSArray * __nonnull RX_TRIVIAextract_arguments(NSInvocation * __nonnull invocation);

/// Returns `YES` in case method has `void` return type.
BOOL RX_TRIVIAis_method_with_description_void(struct objc_method_description method);

/// Returns `YES` in case methodSignature has `void` return type.
BOOL RX_TRIVIAis_method_signature_void(NSMethodSignature * __nonnull methodSignature);

/// Default value for `RXInterceptionObserver.targetImplementation`.
IMP __nonnull RX_TRIVIAdefault_target_implementation(void);
